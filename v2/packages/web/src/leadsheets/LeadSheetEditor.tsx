...
export function LeadSheetEditor({ baseline, initialize = false, capabilities, online, onClose }: LeadSheetEditorProps) {
  ...
  const workspaceAhead = state.revision !== null && state.source !== state.revision.document.source;
  return <article className="detail-page lead-editor" aria-busy={busy}>
    ...
    <ul className="editor-state-list" aria-label="Lead-sheet readiness"><li>Local: {workspaceAhead ? "a newer workspace is preserved and not queued" : "workspace and committed revisions are durable"}</li><li>Local checks: {state.validation.valid ? "pass" : "errors prevent queueing"}</li><li>Queued: {state.queued > 0 ? `${state.queued} revision${state.queued === 1 ? "" : "s"} waiting or retrying` : "none"}</li><li>Acknowledged: {state.baseServerRevisionId === "" ? "no server revision accepted" : state.queued > 0 ? "an older server revision only" : workspaceAhead ? "current server revision acknowledged; newer local workspace remains unqueued" : "latest completed foreground sync"}</li><li>Server/Apex validated: {serverValidated ? "yes, for this exact source" : state.serverValidation === null ? "not for this exact source" : "failed for this exact source"}</li><li>Conflicted: {state.conflicts > 0 ? <a href="#/conflicts">review both retained candidates</a> : "no open conflict"}</li><li>Published: {state.conflicts > 0 ? "conflict candidate is not published" : state.publishedRevisionId === "" ? "not published" : state.publishedRevisionId === state.baseServerRevisionId && state.queued === 0 ? "current acknowledged server revision" : "older revision"}</li></ul>
