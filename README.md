...
**Opt-in V2 writable Set List pilot:** https://kgl-songs.exe.xyz:8003/#/
...
V2 Phase 0 completed its architecture evidence review on August 9, 2026 with a **conditional go** for an isolated read-only PWA. TASK-008 froze the current baseline, TASK-009 projected all 373 documents and 1,076 Set Entries losslessly, TASK-010 produced the reviewed bootstrap API, TASK-011 delivered the isolated verified React/Vite shell, TASK-012 atomically activates and cold-restarts the snapshot from IndexedDB, TASK-013 provides deterministic active-generation offline library/search/status behavior, TASK-014 runs complete locked Set List performances offline, TASK-015 hardens the integrated slice across current-release Chromium routes, accessibility, worker updates, pointer races, and failure recovery, and TASK-016 packages the software checkpoint plus physical-device runbook/checklist. TASK-017 and TASK-018 provide the production owner/device sync ledger and fenced Git publication/recovery foundation; TASK-019 adds the offline writable Set List editor, durable outbox, and authored-state recovery; TASK-020 adds separately gated lead-sheet workspaces/revisions and review-only enrichment candidates; TASK-021 adds durable explicit conflict resolution, stale-cursor/resnapshot recovery, and writable recovery procedures. On August 23, 2026, the owner explicitly authorized an opt-in writable Set List pilot, now served from the compact port-8003 origin. Set List writing and foreground sync are active; lead-sheet writing and enrichment remain off. Formal TASK-021 physical acceptance, default-route cutover, and V1 retirement remain pending.
