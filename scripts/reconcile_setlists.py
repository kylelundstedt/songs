...
support that conclusion; partial dates deliberately never auto-merge.
"""
from __future__ import annotations
import hashlib,json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]
SONGS=ROOT.parent/'songs'
OUT=ROOT/'candidates/reconciled-set-lists.json'; REPORT=ROOT/'reports/reconciliation.md'

def load(p): return json.loads(Path(p).read_text())
def norm(s): return re.sub(r'[^a-z0-9]+',' ',(s or '').lower()).strip()
def full_date(s):
    return isinstance(s,str) and bool(re.fullmatch(r'\d{4}-\d\d-\d\d(?:T.*)?',s))
def day(s): return s[:10] if full_date(s) else None
...
def main():
    ...
    for n,rs in enumerate(sorted(buckets.values(),key=lambda x:min(z['source_id'] for z in x)),1):
        ...
        if not primary['date_exact']: blockers.append('partial or missing date; not eligible for automatic historical-gig merging')
