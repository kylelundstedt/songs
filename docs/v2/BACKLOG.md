...
| P0 | [TASK-012 — production IndexedDB snapshot activation](tasks/TASK-012-production-indexeddb-snapshot.md) | Stage and verify chunks, atomically activate one generation, retain the previous snapshot through failures, and enable offline restart without exposing writes. |
